...
import { associateFetchIdToTags, parseCommaSeparatedTagString } from './cache-tags';
...
export async function executeQuery(query = '', variables = {}) {
  ...
  const fetchId = createHash('sha1').update(query).update(JSON.stringify(variables)).digest('hex');
